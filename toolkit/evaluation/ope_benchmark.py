...
class OPEBenchmark:
    ...
    def overlap_for_tune(self,gt_bb, result_bb, n_frame):
    ...
    def eval_for_tune(self, eval_trackers=None):
        """
        Args:
            eval_trackers: list of tracker name or single tracker name
        Return:
        ...
        for tracker_name in eval_trackers:
            ...
            for video in self.dataset:
                ...
                if hasattr(video, 'absent'):
                    gt_traj = gt_traj[video.absent == 1]
                    tracker_traj = tracker_traj[video.absent == 1]
                iou_[video.name] = self.overlap_for_tune(gt_traj, tracker_traj, n_frame)
            iou[tracker_name] = iou_
        return iou
