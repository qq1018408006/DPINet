...
class Video(object):
    def __init__(self, name, root, video_dir, init_rect, img_names,
        ...
        self.attr = attr
        self.pred_trajs = {}
        ##修改了中间path以适应本地的got10k dataset
        if 'not_used' in root:##固态盘里的test文件分布为标准版
            mid_dir=self.video_dir.split('-')[0]
            self.img_names = [os.path.join(root,mid_dir,mid_dir, x) for x in img_names]
        else:
