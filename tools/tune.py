...
def eval(dataset, tracker_name):
    ...
    if 'OTB' in args.dataset:
        ...
        dataset.set_tracker(tracker_dir, trackers)
        benchmark = OPEBenchmark(dataset)
        eval_iou = benchmark.eval_for_tune(tracker_name)
        iou = np.mean(np.concatenate(list(eval_iou[tracker_name].values())))
        return iou
...
def objective(trial):
    # different params
    cfg.TRACK.WINDOW_INFLUENCE = trial.suggest_uniform('window_influence', 0.000, 1.000)
    cfg.TRACK.PENALTY_K = trial.suggest_uniform('penalty_k', 0.000, 1.000)
    cfg.TRACK.LR = trial.suggest_uniform('scale_lr', 0.000, 1.000)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--config', default='config1.yaml', type=str, help='config file')
    ...
    study = optuna.create_study(study_name='post_process_ph', storage='sqlite:///{}.db'.format(args.dataset),load_if_exists=True,direction='maximize')
    try:
        #print(study.get_trials()),sampler=RandomSampler()
